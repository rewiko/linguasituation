...
class Users extends Front_Controller
{
	 ...
	 public function login_mobile()
	{
		...
		if ($this->auth->is_logged_in() === FALSE)
		{
				...
				$remember = $this->input->get('remember_me') == '1' ? TRUE : FALSE;
				$response=$this->auth->login_mobile($this->input->get('login'), $this->input->get('password'), $remember);
				echo 'jsoncallback('.json_encode($response).')';
				// Try to login
				if ($this->auth->login_mobile($this->input->get('login'), $this->input->get('password'), $remember) === TRUE)
				{
