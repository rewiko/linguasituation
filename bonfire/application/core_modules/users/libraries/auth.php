...
class Auth
{
	...
	/**
	 * Mise en place lmogin via Site mobile 
	 * 
	 *
	 */
	 
	 public function login_mobile($login=NULL, $password=NULL, $remember=FALSE)
	{
		if (empty($login) || empty($password))
		{
			$data['erreur']='empty login or password';
		...
		if($user == FALSE)
		{
			...
			$data['erreur']='username or email or password doesn\'t exist';
			return $data;
		}
		...
		if ($user->active == 0 && $activation_type > 0) // in case we go to a unix timestamp later, this will still work.
		{
			if ($activation_type == 1)
			{
				$data['erreur']='compte non activer';
				
			}
			elseif ($activation_type == 2)
			{
				$data['erreur']='compte pending';
			}
			return $data;
		}
		...
		if ($user->deleted >= 1) // in case we go to a unix timestamp later, this will still work.
		{
			$data['erreur']='compte deleted soft';
			return $data;
		}
